...
export default function PageTags({ tags, center = false }: { tags: string[]; center?: boolean }): JSX.Element {
  return (
    <div className={cn('mb-6 flex flex-wrap items-center gap-3', center ? 'justify-center' : '')}>
      {tags.map((tag: string, index: number) => (
        <span
          key={index}
          className="border-primary/40 inline-flex items-center rounded-2xl border bg-white px-3 py-1.5 font-mono text-[11px] font-semibold"
        >
          {tag}
        </span>
      ))}
