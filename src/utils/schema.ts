...
export function softwareSourceCodeSchema({
  ...
  return {
    ...
    programmingLanguage: { '@type': 'ComputerLanguage', name: programmingLanguage },
...
export function generateMasterSchema(options: MasterSchemaOptions): Array<AnyObject> {
  ...
  if (pageType === 'AboutPage' || pageType === 'ContactPage') mainEntityId = `${rootUrl}#organization`;
  ...
  const fullBreadcrumbs = isHome
    ? []
    : [{ name: 'Home', path: '' }, ...breadcrumbs.filter((b) => b.path !== '' && b.path !== '/')];
    ...
    ...(pageType === 'Software'
      ? [
          isApp
            ...
            : softwareSourceCodeSchema({ name: title, description, path, ...softwareConfig }),
        ]
      : []),
